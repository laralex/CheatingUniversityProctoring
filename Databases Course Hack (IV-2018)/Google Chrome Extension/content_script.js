/*  
	This script is executed in Google Chrome's isolated JavaScript space,
	but since it's a "Content script inside of Google Chrome extension",
	it can read/modify DOM of Google Chrome tabs.
...
chrome.runtime.onMessage.addListener(
	function (request, sender, sendResponse) {
		...
		if (request.action == "Search") {
			sendResponse(SearchFor(request.search_str));
		}
	}
);

function CompareStrings(question1, question2) {
...
function SearchFor(question){
	let result = []
	for (a in answers) {
		if (CompareStrings(question, answers[a]['q']) ) 
...
(()=>{
	...
	for (let qtext = 0; qtext < qtexts.length; qtext++) {
		...
		for (a in answers) {
			if (CompareStrings(tmp.textContent, shpora[a]['q']) ) 
