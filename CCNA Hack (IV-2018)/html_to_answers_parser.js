...
jQuery.get( "https://raw.githubusercontent.com/laralex/UniversitySmallStuff/master/CCNA%20answers%20html%20(1-11%2C%20Final%20Exam%2C%20Practice%20Exam).txt", function( data ) {
...
for (var val in splited) {
	...
	if (question_block != null) {
		...
		quiz[question_text] = answers ? answers.map(function(cur) { 
														// Take just the text
...
Subnetwork C: Printers – 2 addresses`] = [`255.255.255.240`];

quiz[`What network service resolves the URL entered on a PC to the IP address of the destination server?`] = [`DNS`];
